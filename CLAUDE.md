...
## iOS app (Capacitor) — LIVE pipeline, submitted to App Store 2026-07-04

`ios/` is the Capacitor iOS project (same appId `com.pokerpreflop.trainer`, **on-device display name** Preflop Camp; the **App Store name is the ASO one: `PreFlop GTO Poker Trainer`**, App ID `6787507770`). **1.0 + both subscriptions are WAITING_FOR_REVIEW** (submitted via the reviewSubmissions API; review 24-48h, auto-release on approval).
...
- **Store page**: trilingual (en-US/zh-Hans/ja) ASO metadata + 30 screenshots + IAP review screenshots all pushed via the ASC API; copy source = `store-assets/listing.md` (App Store section), screenshots = `tools/gen-ios-shots.js` → `store-assets/ios/`. Categories: Education (primary) / Games>Card (secondary); age rating honestly declares simulated gambling (frequent).
