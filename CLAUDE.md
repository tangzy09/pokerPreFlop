...
## iOS app (Capacitor) — cloud-built, no Mac

`ios/` is the Capacitor iOS project (same appId `com.pokerpreflop.trainer`, display name **Preflop Camp**, `ITSAppUsesNonExemptEncryption=false`, 1024 AppIcon from `tools/gen-store-assets.py`). **Building requires macOS — this repo uses the free GitHub Actions macOS runner instead of a local Mac**: `.github/workflows/ios-build.yml` (manual trigger; default = unsigned sanity build with zero secrets, `upload=true` + 6 repo secrets = signed archive → TestFlight). Full account/ASC/RevenueCat/signing checklist in [IOS_SETUP.md](IOS_SETUP.md). `js/purchases.js` is already cross-platform (`apiKey()` branches per platform); `RC_API_KEY.ios` is a placeholder until RevenueCat's App Store app is created. iOS signing material (`*.p8/*.p12/*.mobileprovision`) is gitignored — repo is public.
