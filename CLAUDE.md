...
- `L("中文")` → current language; **missing → English**; last resort → the Chinese source. ⚠ It must **never** fall back to the Chinese source for a non-zh user (a French user would see Chinese). `_walkText`/`_walkAttr` route through `L()` too, so static HTML needs no per-language markup. Used for short labels, action names, spot `name`/`who` (the latter via `Lwho`/`Lparts` in app.js, which split on ` · ` / spaces — ⚠ per-token translation is the "concatenated fragment" anti-pattern; 7 sites, fine for list-like labels, audit before relying on it for word-ordered languages).
